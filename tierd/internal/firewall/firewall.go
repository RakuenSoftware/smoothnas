...
func GenerateRuleset(enabledProtocols map[string]bool) string {
	...
	b.WriteString("#\n")
	b.WriteString("# Replace ONLY tierd's own table. A blanket ruleset flush would\n")
	b.WriteString("# also wipe the plugin runtime's tables (ip veth_nat holds the\n")
	b.WriteString("# DNAT rules that publish hostExpose ports like Wolf's Moonlight\n")
	b.WriteString("# ports and the Wolf Den UI), so re-applying the host firewall\n")
	b.WriteString("# while a plugin is running would drop its published ports until\n")
	b.WriteString("# the container restarted. Scoping to `table inet filter` makes\n")
	b.WriteString("# firewall apply idempotent and independent of the runtime.\n")
	b.WriteString("# The bare `table inet filter` line ensures it exists so the\n")
	b.WriteString("# delete cannot fail on first apply; the whole script loads\n")
	b.WriteString("# atomically under `nft -f`.\n\n")
	b.WriteString("table inet filter\n")
	b.WriteString("delete table inet filter\n\n")
