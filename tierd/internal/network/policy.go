...
const (
	...
	// DefaultBondVethUnmanagedFilename keeps transient LXC/container
	// veth links from matching the default-bond catch-all member file.
	// Its low prefix makes it the first matching .network file for
	// veth* interfaces.
	DefaultBondVethUnmanagedFilename = "01-veth-unmanaged.network"
...
// GenerateDefaultBondVethUnmanagedNetwork returns the first-match
// systemd-networkd rule that keeps transient LXC/container veth links
// out of the default bond catch-all. systemd-networkd may classify a
// host-side veth as Type=ether early enough for broad Ethernet matches;
// an explicit unmanaged veth* file prevents those links from being
// enslaved to bond0.
func GenerateDefaultBondVethUnmanagedNetwork() string {
	var b strings.Builder
	b.WriteString("# Auto-generated by tierd default-bond policy. Do not edit.\n")
	b.WriteString("# Keeps transient LXC/container veth interfaces out of bond0.\n")
	b.WriteString("[Match]\n")
	b.WriteString("Name=veth*\n")
	b.WriteString("\n")
	b.WriteString("[Link]\n")
	b.WriteString("Unmanaged=yes\n")
	return b.String()
}
...
func refreshDefaultBondMembersIfPresent(networkDir string) error {
	...
	if err := WriteConfigFile(networkDir, DefaultBondVethUnmanagedFilename, GenerateDefaultBondVethUnmanagedNetwork()); err != nil {
		return fmt.Errorf("write default bond veth unmanaged: %w", err)
	}
...
func writeDefaultBondFiles(store ConfigStore, networkDir, sysRoot string) error {
	...
	if err := WriteConfigFile(networkDir, DefaultBondVethUnmanagedFilename,
		GenerateDefaultBondVethUnmanagedNetwork()); err != nil {
		return fmt.Errorf("write default bond veth unmanaged: %w", err)
	}
...
func BreakBond(networkDir, bondName string, members []string) error {
	...
	// Drop the catch-all files that the default-bond policy uses.
	// Without this step a freshly-plugged NIC after Break Bond would
	// still try to join `bond0`, which no longer exists.
	for _, fn := range []string{DefaultBondVethUnmanagedFilename, DefaultBondMembersFilename} {
		if err := RemoveConfigFiles(networkDir, fn); err != nil {
			return fmt.Errorf("remove default-bond file %s: %w", fn, err)
		}
	}
