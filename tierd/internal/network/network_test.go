...
func TestValidateDNSServer(t *testing.T) {
	...
	invalid := []string{"", "not-an-ip", "example.com", "999.999.999.999"}
...
func TestParseResolvectlDNSSkipsLocalStub(t *testing.T) {
	out := `Global: 127.0.0.53
Link 4 (bond0): 192.168.1.1 1.1.1.1
Link 5 (net1): 1.1.1.1 2001:4860:4860::8888
`
	got := strings.Join(parseResolvectlDNS(out), ",")
	want := "192.168.1.1,1.1.1.1,2001:4860:4860::8888"
	if got != want {
		t.Fatalf("servers = %q want %q", got, want)
	}
}

func TestParseResolvectlDomainsSkipsRouteOnlyDomains(t *testing.T) {
	out := `Global: ~.
Link 4 (bond0): lan example.com ~corp.example
`
	got := strings.Join(parseResolvectlDomains(out), ",")
	want := "lan,example.com"
	if got != want {
		t.Fatalf("domains = %q want %q", got, want)
	}
}

func TestGenerateResolvedDNSDropIn(t *testing.T) {
	content := GenerateResolvedDNSDropIn(DNSConfig{
		Servers:       []string{"1.1.1.1", "2001:4860:4860::8888"},
		SearchDomains: []string{"lan"},
	})
	if !strings.Contains(content, "[Resolve]") {
		t.Fatalf("missing [Resolve] section:\n%s", content)
	}
	if !strings.Contains(content, "DNS=1.1.1.1 2001:4860:4860::8888") {
		t.Fatalf("missing DNS servers:\n%s", content)
	}
	if !strings.Contains(content, "Domains=lan") {
		t.Fatalf("missing search domain:\n%s", content)
	}
}

func TestSetDNSWithRunnerWritesDropInAndRestartsResolved(t *testing.T) {
	dir := t.TempDir()
	var command string
	err := setDNSWithRunner(DNSConfig{
		Servers:       []string{"1.1.1.1"},
		SearchDomains: []string{"lan"},
	}, dir, func(name string, args ...string) ([]byte, error) {
	...
	data, err := os.ReadFile(filepath.Join(dir, smoothNASResolvedConfigFile))
	if err != nil {
		t.Fatalf("read drop-in: %v", err)
	}
	content := string(data)
	if !strings.Contains(content, "DNS=1.1.1.1") || !strings.Contains(content, "Domains=lan") {
		t.Fatalf("unexpected drop-in:\n%s", content)
	}
	if command != "systemctl restart systemd-resolved" {
		t.Fatalf("restart command = %q", command)
	}
}

func TestParseNetworkFileSettingsKeepsPerConnectionDNS(t *testing.T) {
	settings := parseNetworkFileSettings([]byte(`# Auto-generated by tierd. Do not edit.
[Match]
Name=bond0
...
MTUBytes=9000
`))
	if !settings.DHCP4 || settings.DHCP6 {
		t.Fatalf("DHCP flags = v4:%v v6:%v", settings.DHCP4, settings.DHCP6)
	}
	if got := strings.Join(settings.IPv4Addrs, ","); got != "192.168.1.254/23" {
		t.Fatalf("IPv4 addrs = %q", got)
	}
	if got := strings.Join(settings.IPv6Addrs, ","); got != "2001:db8::10/64" {
		t.Fatalf("IPv6 addrs = %q", got)
	}
	if settings.Gateway4 != "192.168.1.1" || settings.Gateway6 != "fe80::1" {
		t.Fatalf("gateways = %q / %q", settings.Gateway4, settings.Gateway6)
	}
	if !settings.SLAAC || settings.MTU != 9000 {
		t.Fatalf("SLAAC/MTU = %v/%d", settings.SLAAC, settings.MTU)
	}
	if got := strings.Join(settings.DNS, ","); got != "192.168.1.1,1.1.1.1" {
		t.Fatalf("DNS = %q", got)
	}
}
